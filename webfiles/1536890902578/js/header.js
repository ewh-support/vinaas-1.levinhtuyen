...
//jQuery for active tab using page url
...
//    var $url = "download";
//    if (arr[1] === $url) {
//      jQuery('.main-nav .get-started .test-drive').click(function () {
//        jQuery("#cloud-test-drive").trigger("click");
//      });
//
//      if (jQuery('.main-nav .get-started .test-drive').hasClass('redirect')) {
...
//  }
    ...
    if (window.location.href.indexOf("download") > -1) {
        jQuery('.main-nav .get-started .test-drive').removeClass('redirect');
    } else {
        jQuery('.main-nav .get-started .test-drive').addClass('redirect');
    }

    var page_url = window.location.pathname;
    if (page_url.indexOf('download') > -1) {
        jQuery('.main-nav .get-started .test-drive').click(function () {
            jQuery("#cloud-test-drive").trigger("click");
...
//  jQuery for active tab using page url
        ...
        jQuery(params).each(function (i, elem) {
//    console.log(params[1]);
            if (elem !== "") {
                elem = elem.split("=");
                jQuery("#" + params[1]).trigger("click");
            }
        });
    }
});
